...
from .pb import quoridor_rl_v8_pb2, quoridor_rl_v8_pb2_grpc

ACTION_SPACE_SIZE = 136  # 8 movimiento (4 cardinal + 4 diagonal) + 64 muros H + 64 muros V
OBSERVATION_SIZE = 491


class QuoridorLeagueEnv(gym.Env):
    """Gymnasium wrapper for the isolated V8 server.

    The Go server emits state from the player-to-move perspective. Python must
    never apply a second vertical flip.
    """

    metadata = {"render_modes": []}

    def __init__(self, grpc_target="localhost:50052", checkpoint_dir="models_quoridor_v8",
                 opponent_sampling="uniform", opponent_pool_cache_size=8, timeout=5.0,
                 model_loader=None):
        super().__init__()
    ...
    def _load_random_historical_opponent(self):
        ...
        self.opponent_checkpoint = chosen
        key = str(chosen)
        if key in self._model_cache:
            self.opponent_model = self._model_cache.pop(key)
            self._model_cache[key] = self.opponent_model
            return
        try:
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self._load_random_historical_opponent()
        self.agent_player_id = random.choice((1, 2))
        self._prev_phi = 0.0
        response = self.stub.Reset(quoridor_rl_v8_pb2.ResetRequest(), timeout=self.timeout)
        response = self._play_opponent_turn(response)
    ...
    SHAPING_COEF = 0.01  # bajo a propósito: no debe dominar sobre +1/-1/-0.1

    def _agent_reward(self, done, info):
        """Reward from the agent's perspective, independent of whether the
        final Step() in Go was made by the agent or by the historical
        opponent inside _play_opponent_turn (response.reward alone is not
        safe to reuse: Go computes it from the perspective of whoever made
        ...
        # Reward shaping basado en potencial (Ng et al. 1999): no cambia la
        # política óptima, solo densifica la señal para acelerar el
        # aprendizaje. phi = qué tan peor está el rival respecto al agente
        # en distancia al objetivo; premiamos que esa diferencia mejore.
    ...
    def _play_opponent_turn(self, response):
        while not response.done and response.current_player != self.agent_player_id:
            ...
            if self.opponent_model is None:
                ...
                action, _ = self.opponent_model.predict(observation, action_masks=mask, deterministic=False)
                action = int(action)
    ...
    def action_masks(self):
        """Return the valid actions using the SB3-Contrib masking API."""
